...
type Ball()=
    ...
    member this.Center
        with get() = new PointF(position.X+diameter/2.f, position.Y+diameter/2.f)
...
type Coin(pos:PointF) = 
    let mutable position = new PointF(pos.X, pos.Y)
    ...
    member this.IsInside(p:PointF):bool=
        pointless <- false
        let xpow = pown (p.X-this.Center.X) 2
        let ypow = pown (p.Y-this.Center.Y) 2
        let distance = sqrt (xpow + ypow)
        distance < diameter/2.f

    member this.collectTest(ball:Ball):bool =
        let ballCenter = ball.Center
        let xpow = pown (ballCenter.X-this.Center.X) 2
        let ypow = pown (ballCenter.Y-this.Center.Y) 2
        let distance = sqrt (xpow + ypow)
        distance < (diameter/2.f) + (ball.Diameter/2.f)
